...
func (n *NodeProxy) SendWithNoRersult(batchMessage *network.BatchMessage) {
	logger.Debugf("--Proxy server received message term:%d GOROUTINE:%d\n", batchMessage.Term, utils.GetGID())
	//s.before(batchMessage)
	count := len(batchMessage.Messages)
	for i := 0; i < count; i++ {
...
func (n *NodeProxy) SendKeyMsg(key string, to uint64, op uint32, data []byte) ([]byte, error) {
	var msg *network.Message
	var term uint64
	term = n.generateId()
